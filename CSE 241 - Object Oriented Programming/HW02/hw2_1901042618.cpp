...
int load(vector< vector<cellState> >&vec , string str_txt , char& gameType , int& numberOfMoves);			   //loads the specified file of board
int save(const vector< vector<cellState> >&vec , string str_txt , const char& gameType, const int& numberOfMoves);//saves the current board and related informations 
...
int is_moveValid(const vector<vector<cellState>>& vec , string move , int rowVariable , int columnVariable);   //controls move input is valid or not
void valid_move_Function(char gameType , vector<vector<cellState>>& vec);									   //makes the move on the board for first 5 board
void boardStyle_1(char gameType);																			   //creates the first board
void boardStyle_2(char gameType);																			   //creates the second board
void boardStyle_3(char gameType);																			   //creates the third board
void boardStyle_4(char gameType);																			   //creates the fourth board
void boardStyle_5(char gameType);																			   //creates the fifth board
int is_anyMovePossible_6(const vector<vector<cellState>>& vec , int row , int column);                         //controls whether any movement possible on sixth board
int is_moveValid_6(const vector<vector<cellState>>& vec , string move , int rowVariable , int columnVariable); //controls move input is valid or not for sixth board
void make_move_6(vector<vector<cellState>>& vec , string move , int rowVariable , int columnVariable);         //makes the move on the sixth board
void valid_move_Function_6(char gameType , vector<vector<cellState>>& vec);									   //
void boardStyle_6(char gameType);																			   //creates the sixth board
...
int main()
{
	...
	vector< vector<cellState> > temp_vec(9, vector<cellState>(9, wall));
	string str_txt;
	...
	do
	{
		...
		if (lors == "LOAD"){													//LOADING 
			...
			load(temp_vec , str_txt , opt_gameType , numberOfMoves);
		}
	...
	do
	{
		...
		if ((str_opt_gameType.size() != 1) || str_opt_gameType[0] < '1' || str_opt_gameType[0] > '2')
			cerr << endl << "TRY AGAIN.." << endl;

	} while ((str_opt_gameType.size() != 1) || str_opt_gameType[0] < '1' || str_opt_gameType[0] > '2');
...
int load(vector< vector<cellState> >&vec , string str_txt , char& gameType , int& numberOfMoves)
{
	int r = 0 , c = 0 , ans = 0;
	int i = 0;
	decltype(i) j = 0;
	...
	if (file)
  	{	
      	...
      	for (i = 0; i < r; ++i)
		{
			...
			{
				file >> ch;
				if(ch == '0') vec[i][j] = empty;
				else if(ch == '1') vec[i][j] = peg;
				else if(ch == '2') vec[i][j] = wall;
			}
		}
		
		cout << "Loaded board is printing.." << endl;
		if (vec[3][6] == wall){											//if board is sixth one
			printVector(vec , vec.size() , vec[0].size() , 6);
		}
		else
			printVector(vec , vec.size() , vec[0].size());

		do
		{
			cout << "1-> Human player game" << endl
				 << "2-> Computer game" << endl
			 	 << "Please select the Game type: ";					//user can change the gameType when new board loaded
			cin >> str_opt_gameType;
		...
		gameType = str_opt_gameType[0];

		if (vec[3][6] == wall){										 //if board is sixth one
			valid_move_Function_6(gameType , vec);
		}
		else
			valid_move_Function(gameType , vec);
...
int save(const vector< vector<cellState> >&vec , string str_txt , const char& gameType , const int& numberOfMoves)
{
	int r = 0 , c = 0 , ans = 0;
	...
	if (str_txt.substr(str_txt.size()-4 , 4) != ".txt"){   //if the file extension is not .txt give error
		...
		for(auto &row : vec){
			c=0;
    		for(auto col : row){
    			file << col;				//prints the vector on txt file
				c++;
			}
			file << endl;
			r++;	  
...
void valid_move_Function(char gameType , vector<vector<cellState>>& vec){
	...
	if (gameType == '1'){ //if game type is human player game
		...
		do
		{
			do{
				...
				if (lors == "LOAD"){								//LOADING 
					...
					flagLoad = load(vec , str_txt , gameType , numberOfMoves);
					//if(flagLoad == 1) printVector(vec , vec.size() , vec[0].size());
				}
				else if (lors == "SAVE"){							//SAVING
					str_txt = move.substr(5,move.length()-5);
					flagSave = save(vec , str_txt , gameType , numberOfMoves);
					if(flagSave == 1){
		...
		do
		{
			...
			} while (!is_moveValid(vec , move_cmp , rondomRow , rondomColumn));

			cout << "press any key for next move:";
			getline(cin >> std::ws, next);   					//ws consumes whitespace from an input stream

			lors = next.substr(0,4);
			if (lors == "LOAD"){								//LOADING 
				str_txt = next.substr(5,next.length()-5);
				flagLoad = load(vec , str_txt , gameType , numberOfMoves);
			}
			else if (lors == "SAVE"){							//SAVING
				str_txt = next.substr(5,next.length()-5);
...
void valid_move_Function_6(char gameType , vector<vector<cellState>>& vec)
{
	...
	if (gameType == '1'){  //if game type is human player game
		do
		{
			do
			{
				...
				if (lors == "LOAD"){								//LOADING 
					...
					if ((move.size() > 4) || !is_firstParameter || !is_secondParameter || !is_thirdParameter || !is_fourthParameter || !is_moveValid_6(vec , move , rowVariable , columnVariable )){
						cerr << "Try Again.." << endl;
						is_firstParameter = 0;
						is_secondParameter = 0;
						is_thirdParameter = 0;
			...
			} while ((move.size() > 4) || !is_firstParameter || !is_secondParameter || !is_thirdParameter || !is_fourthParameter || !is_moveValid_6(vec , move , rowVariable , columnVariable ));
		
			make_move_6(vec , move , rowVariable , columnVariable);

		} while (is_anyMovePossible_6(vec , 9 , 13));					//number of rows of the vector=9 , number of columns of the vector=13
		...
		do
		{
			...
			} while (!is_moveValid_6(vec , move , rondomRow , rondomColumn));
			...
			make_move_6(vec , move , rondomRow , rondomColumn);
			cout << "move made: " << move << endl;
			
		} while (is_anyMovePossible_6(vec , 9 , 13));
...
void boardStyle_6(char gameType)
{
	...
	valid_move_Function_6(gameType , myVector_6);
