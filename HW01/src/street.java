 ...
 * that will be used for designing a small one street town.
 * @author Ali KAYA
 * @version 1.0
 * @since 2022-03-06
 */
public class street {
    ...
    public int add(int borf, buildTypes newBuild) {
        ...
        switch (borf)
        {
            case 0:
                ...
                {
                    ans = 1;
                    if (getBuildingNumsBack() >= getCapBack()) {
                        setCapBack(getCapBack() + 10);

                        buildTypes[] temp = (buildTypes[]) new Object[getCapBack()];
                    ...
                    arr[borf][getBuildingNumsBack()] = newBuild;
            ...
            case 1:
                ...
                {
                    ans = 1;
                    if (getBuildingNumsFront() >= getCapFront()) {
                        setCapFront(getCapFront() + 10);

                        buildTypes[] temp = new buildTypes[getCapFront()];
                    ...
                    arr[borf][getBuildingNumsFront()] = newBuild;
    ...
    public int delete(int borf) {
        ...
        if (borf == 0 && getBuildingNumsBack() != 0) {
            setStreetLengthEmptyBack(getStreetLengthEmptyBack() + arr[borf][getBuildingNumsBack() - 1].getLength());
            arr[borf][getBuildingNumsBack() - 1] = null;
            setBuildingNumsBack(getBuildingNumsBack() - 1);
        }
        else if (borf == 1 && getBuildingNumsFront() != 0) {
            setStreetLengthEmptyFront(getStreetLengthEmptyFront() + arr[borf][getBuildingNumsFront() - 1].getLength());
            arr[borf][getBuildingNumsFront() - 1] = null;
            setBuildingNumsFront(getBuildingNumsFront() - 1);
        }
        else
            ans = 0;
    ...
    public boolean isEmptyArea(int borf, buildTypes newBuild) {
        ...
        for (int i = 0; i < length; ++i) {
            if ((newBuild.getPosition() >= arr[borf][i].getPosition()) && (newBuild.getPosition() < arr[borf][i].getPosition() + arr[borf][i].getLength())) {
                ans = false;
                break;
            }
        }
    ...
    public void displayListOfBuildings()
    {
        ...
        for (i = 0; i < 2; ++i)
        {
            ...
            while (counter < breakCounter) {
                ...
                if (arr[i][counter - 1] instanceof house) {
                    System.out.printf("\nFeatures of %s's house ;\n Owner printing with focus method!\n Color: %s\n Number of rooms: %s\n " +
                                    "Position: %s\n Height of the house: %s\n Length of the house: %s\n"
                            , arr[i][counter-1].focus()
                            , ((house) arr[i][counter-1]).getColor()
                            , ((house) arr[i][counter-1]).getRoomNum()
                            , arr[i][counter-1].getPosition()
                            , arr[i][counter-1].getHeight()
                            , arr[i][counter-1].getLength());
                }
                else if (arr[i][counter - 1] instanceof market) {
                    System.out.printf("\nFeatures of %s's market ;\n Opening time: %s\n " +
                                    "Closing time: %s (printing with focus method)\n Position: %s\n Height of the market: %s\n Length of the market: %s\n "
                            , ((market) arr[i][counter-1]).getOwner()
                            , ((market) arr[i][counter-1]).printOpenHour()
                            , arr[i][counter-1].focus()
                            , arr[i][counter-1].getPosition()
                            , arr[i][counter-1].getHeight()
                            , arr[i][counter-1].getLength());
                }
                else if (arr[i][counter - 1] instanceof office) {
                    System.out.printf("\nFeatures of %s's office ;\n Job Type of the office: %s (printing with focus method)\n Position: %s\n " +
                                    "Height of the office: %s\n Length of the office: %s\n"
                            , ((office) arr[i][counter-1]).getOwner()
                            , arr[i][counter-1].focus()
                            , arr[i][counter-1].getPosition()
                            , arr[i][counter-1].getHeight()
                            , arr[i][counter-1].getLength());
                ...
                else if (arr[i][counter - 1] instanceof playground) {
    ...
    public void displayRatioPlayground()
    {
        ...
        for (i = 0; i < 2; ++i) {
            ...
            while (counter < breakCounter) {
                ...
                if (arr[i][counter - 1] instanceof playground) {
                    lengthCounter += arr[i][counter - 1].getLength();
                }
            }
        }
    ...
    public void displayTotalOccupied()
    {
        ...
        for (i = 0; i < 2; ++i) {
            ...
            while (counter < breakCounter) {
                ...
                if (arr[i][counter - 1] instanceof house)
                    houseLengths += arr[i][counter - 1].getLength();

                else if (arr[i][counter - 1] instanceof market)
                    marketLengths += arr[i][counter - 1].getLength();

                else if (arr[i][counter - 1] instanceof office)
                    officeLengths += arr[i][counter - 1].getLength();
            }
        }
    ...
    public void displaySilhouette()
    {
        ...
        int breakCounter = 0 , counter;
        int startPos , endPos;
        int maxHeight = arr[1][0].getHeight();
        ...
        for (i = 0; i < 2; ++i) {
            ...
            while (counter < breakCounter) {
                counter++;
                if (arr[i][counter - 1].getHeight() > maxHeight) {
                    maxHeight = arr[i][counter - 1].getHeight();
                }
            }
        }
        ...
        for (i = 0; i < 2; ++i) {
            ...
            while (counter < breakCounter) {
                ...
                startPos = arr[i][counter - 1].getPosition();
                endPos = arr[i][counter - 1].getPosition() + arr[i][counter - 1].getLength();


                for (int j = startPos; j < endPos; j++) {
                    if(!overlapController(j , arr[i][counter - 1].getHeight())) myArr[maxHeight - arr[i][counter - 1].getHeight()][j] = '_';
                }
            }
        }
        for (int j = 0; j < myArr.length; j++) {
            for (int k = 0; k < myArr[i].length; k++) {
                if (myArr[j][k] == '_' && (k == 0 || k == myArr[i].length-1)){
    ...
    public boolean overlapController(int myPos, int myHeight) {
        ...
        if (posCounter == 2)
        {
            if(arr[xIndex_1][yIndex_1].getHeight() > arr[xIndex_2][yIndex_2].getHeight())
                minHeight = arr[xIndex_2][yIndex_2].getHeight();
            else
                minHeight = arr[xIndex_1][yIndex_1].getHeight();
    ...
    public static void menu()
    {
        ...
        street myStreet = new street(modeSelect);
